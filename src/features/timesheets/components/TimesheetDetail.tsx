...
import { ArrowLeft, ChevronLeft, ChevronRight, Plus, X, CheckCircle, XCircle, CalendarClock, Clock, Building, Calendar } from 'lucide-react';
    ...
    return (
        ...
        >
            <div className="space-y-6">
                ...
                <div className="relative overflow-hidden rounded-3xl border border-teal-500/20 shadow-xl px-8 py-6 flex-shrink-0"
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row justify-between gap-6">
                        <div className="flex items-start gap-4">
                            <div className="w-14 h-14 rounded-2xl bg-white/30 flex items-center justify-center flex-shrink-0 border border-white/50 backdrop-blur-md shadow-sm">
                                <CalendarClock className="w-7 h-7 text-slate-900" />
                            </div>
                            <div>
                                <div className="flex items-center gap-3 mb-1">
                                    <h1 className="text-[28px] leading-none tracking-tight font-black text-slate-900">{timesheet.name}</h1>
                                </div>
                                <div className="flex flex-wrap items-center mt-3 gap-4 text-[13px] font-bold text-slate-800">
                                    <span className="flex items-center gap-1.5 bg-white/20 px-3 py-1 rounded-full"><Clock className="w-4 h-4 opacity-70" /> {Number(timesheet.total_hours || 0).toFixed(2)} Total Hours</span>
                                    <span className="flex items-center gap-1.5 bg-white/20 px-3 py-1 rounded-full"><Building className="w-4 h-4 opacity-70" /> {timesheet.billing_type || 'Unknown Billing'}</span>
                                    <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4 opacity-70" /> {dateRangeLabel}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-6">
                            <div className="text-right flex flex-col items-end">
                                <p className="text-[12px] font-black text-slate-800 mb-2 uppercase tracking-widest">Approval Status</p>
                                <div className={`px-4 py-1.5 rounded-full text-[13px] font-black uppercase tracking-wider shadow-sm border ${
                                    timesheet.approval_status === 'Approved' ? 'bg-[#D1FAE5] text-[#065F46] border-[#A7F3D0]' :
                ...
                {/* Toolbar */}
                <div className="card-base rounded-2xl py-3 px-5 flex flex-wrap items-center justify-between gap-4 shadow-sm border border-slate-200/60 dark:border-slate-800/60">
                    <div className="flex items-center gap-4 flex-wrap w-full md:w-auto">
                        ...
                        <div className="flex bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
                            {(['day', 'week', 'month'] as ViewMode[]).map(mode => (
                                <Button
                                    ...
                                    text={viewMode !== mode}
                                    className={`!px-4 !py-1.5 !text-[12px] font-bold capitalize transition-all ${viewMode === mode ? 'btn-gradient !shadow-sm' : '!text-slate-500 hover:!text-slate-700 dark:hover:!text-slate-300'}`}
                                    label={mode}
                                />
                            ))}
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            <Button 
                                icon={<ChevronLeft className="w-4 h-4 text-slate-500" />} 
                                onClick={() => navigateDate('prev')} 
                                text
                                className="!p-1.5 !w-8 !h-8 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" 
                            />
                            <span className="text-[13px] font-bold text-slate-700 dark:text-slate-300 min-w-[180px] text-center">{dateRangeLabel}</span>
                            <Button 
                                ...
                                {rows.map(row => (
                                    <tr key={row.rowId} className="border-b hover:bg-[#FAFBFC] group">
                                        ...
                                        <td className="text-right px-3 py-3 font-semibold text-[#1F2937]">
                                            {getRowTotal(row) > 0 ? Number(getRowTotal(row)).toFixed(2) : '—'}
                                        </td>
                                ...
                                <tr className="border-b">
                                    ...
                                    {dates.map(date => (
                                        <td key={date} className={`text-center px-1 py-3 font-semibold border-r ${[0, 6].includes(new Date(date + 'T00:00:00').getDay()) ? 'bg-[#F9FAFB]/50' : ''}`}>
                                            {dailyTotals[date] > 0 ? <span className="text-theme-primary">{Number(dailyTotals[date]).toFixed(2)}</span> : <span className="text-transparent">—</span>}
                                        </td>
                                    ))}
