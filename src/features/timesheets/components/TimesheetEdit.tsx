...
export function TimesheetEdit() {
        ...
        const loadData = async () => {
            try {
                const [usersData, projectsData, tsData] = await Promise.all([
                    ...
                    timesheetsService.getTimesheet(Number(id))
                ]);
                setUsers(usersData);
                ...
                if (tsData) {
                    ...
                    // Set current date to the start_date to jump the calendar there
                    if (tsData.start_date) {
                        setCurrentDate(new Date(tsData.start_date));
                    }
                }
            } catch (err) {
                console.error("Failed to load timesheet data", err);
                showToast('error', 'Error', 'Failed to load timesheet');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showToast('success', 'Timesheet Updated', `Timesheet span saved for ${fmtNice(dateRange.start)} to ${fmtNice(dateRange.end)}`);
            navigate('/timesheets');
        } catch (error) {
            console.error(error);
            showToast('error', 'Error', 'Failed to update timesheet');
        } finally {
                ...
                <Card title="Time Period Details">
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-[13px] font-medium text-[#374151] mb-1">Approval Status</label>
                                <select
