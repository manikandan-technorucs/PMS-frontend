                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mt-6">
                  {files.map((f, i) => (
                    <div key={i} className="relative group bg-theme-surface border border-theme-border rounded-lg overflow-hidden flex flex-col items-center p-2 shadow-sm">
                      ...
                      <Button unstyled type="button" onClick={() => removeFile(i)} className="absolute top-1 right-1 p-1 bg-red-500 text-white rounded opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600">
                        <X className="w-3 h-3" />
                      </Button>
                    </div>
                  ))}
