      ...
      {loading ? (
        ...
        <div className="h-full flex flex-col overflow-hidden space-y-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 flex-shrink-0">
            <StatCard label="Total Milestones" value={stats.total} icon={<MilestoneIcon className="w-5 h-5 text-gray-500" />} accent={false} className="bg-white border" />
            <StatCard label="Completed" value={stats.completed} icon={<CheckCircle className="w-5 h-5 text-brand-teal-600" />} className="bg-white border border-t-brand-teal-500" />
            <StatCard label="In Progress" value={stats.inProgress} icon={<Clock className="w-5 h-5 text-blue-600" />} className="bg-white border border-t-blue-500" />
            <StatCard label="Pending" value={stats.pending} icon={<AlertCircle className="w-5 h-5 text-amber-600" />} className="bg-white border border-t-amber-500" />
          </div>

          {view === 'list' ? (
            <div className="flex-1 overflow-auto bg-white dark:bg-slate-900 rounded-xl border shadow-sm">
              <DataTable
                columns={columns}
