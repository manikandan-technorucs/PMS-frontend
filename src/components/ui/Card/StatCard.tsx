...
export function StatCard({ label, value, icon, accent = true, className = '' }: StatCardProps) {
    return (
        <div className={`rounded-xl p-5 flex flex-col justify-between shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 relative overflow-hidden ${className}`} style={{ backgroundColor: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
            ...
            <div className="flex items-start justify-between relative z-10">
                <div className="flex-1">
                    <p className="text-[11px] mb-2 uppercase tracking-[0.05em] font-extrabold text-theme-muted">{label}</p>
                    <p className="text-[32px] font-black text-theme-primary leading-tight tracking-tight">{value}</p>
                </div>
                {icon && (
